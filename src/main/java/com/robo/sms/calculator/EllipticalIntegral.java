...
public class EllipticalIntegral{
    static final double PI_4 = 0.7853981633974483096156608458198757; // pi/4
    static final double PI_2 =  1.5707963267948966192313216916397514; // pi/2
    static final double PI   =  3.1415926535897932384626433832795029; // pi
    static final double LDBL_EPSILON = 0.00001;
    
   public static double GetArcLengthOfEllipse(double a , double b , double angle){
       double fi = Math.atan(a / (b * Math.tan(angle)));
       double m = 1 - Math.pow(a/b, 2);
       double elipticalIntegral = Legendre_Elliptic_Integral_Second_Kind(fi, 'm', m);
       double arcLength = b * elipticalIntegral;
       return arcLength;
   }
    
    ////////////////////////////////////////////////////////////////////////////////
    //http://www.mymathlib.com/c_source/functions/elliptic_integrals/legendre_elliptic_integral_second_kind.c
...
public static double Legendre_Elliptic_Integral_Second_Kind(double amplitude, char arg,
                                                                      double x)
{
   double phi;
   double k, m;
 ...
 // Convert modulus, modular angle, or parameter to modulus and parameter. //
   ...
   switch (arg) {
      ...
      case 'm': m = (double) x;
                ...
                break;
      case 'a': k = Math.sin((double)x);
                m = k * k;
                break;
      default:  k = Math.abs((double) x);
                m = k * k;
   }
   ...
   if ( m > 0.0 && m < 1.0 ) {
      Elliptic_Integral_Second_Kind( Math.abs((double) amplitude), m,Em, Em_phi);
   ...
   if (m < 0.0 ) {
      phi = PI_2 - Math.abs((double) amplitude);
      Elliptic_Integral_Second_Kind( Math.abs(phi), Math.abs(m / (1.0 - m)),
                                                                 Em, Em_phi);
      if (phi > PI_2)
         return (double) (sgn_amplitude * (Em + Em_phi) * Math.sqrt(1.0 - m));
      else 
         return (double) (sgn_amplitude * (Em - Em_phi) * Math.sqrt(1.0 - m));
   }
   ...
   if ( m == 1.0 ) {
      if ( Math.abs(amplitude) <= M_PI_2) return Math.sin(amplitude);
      n = (int) ( (amplitude + M_PI_2) / Math.PI );
      n += n;
      return (double) n + Math.sin(amplitude - n * Math.PI);
   }

                   // Check for the remaining case m > 1. //

   return sgn_amplitude * Large_Modulus(Math.abs(amplitude), k);
...
public static void Elliptic_Integral_Second_Kind(double phi,double m,Double Em,Double Em_phi)
{
   Double F,Fk;
   F = Double.valueOf(0);
   Fk = Double.valueOf(0);
   ...
   phi -= n * PI;
   n += n;
   Landen_Transform( Math.abs(phi), m, F, Fk, Em_phi, Em);
...
public static double Large_Modulus(double amplitude,double k)
{
   Double F, K, E, Ek;
   F = Double.valueOf(0);
   K = Double.valueOf(0);
   ...
   if ( Math.abs(sin_phi) >= 1.0 / k ) {
      if (phi > 0.0)
        phi = PI_2;
      else 
        phi = -PI_2;
   }else {
       phi = Math.asin(k * sin_phi);
   }
   Landen_Transform( Math.abs(phi), 1.0/(k*k),F, K, E, Ek);
   Ek = k * Ek + (1.0 - k*k) * K / k;
   E = k * E + (1.0 - k*k) * F / k;
   if (phi >= 0.0) E +=  n * Ek;
   else E = n * Ek - E;
...
public static void Landen_Transform(double phi,double parameter,
                                 Double F, Double Fk, Double E, Double Ek) 
{ 
    double two_n = 1.0;
    double a = 1.0;
    double g = Math.sqrt(1.0 - parameter);
    double a_old;
    double g_old;
    double tan_2n_phi;
    double sum = 2.0 * (2.0 - parameter);
    double integral = 0.0;
   ...
   while (true) {
      ...
      sum -= two_n * (a - g) * (a - g);
      two_n += two_n;
      phi -= Math.atan((a - g) * tan_2n_phi / (a + g * tan_2n_phi * tan_2n_phi) )
                                                                       / two_n;
      ...
      if ( Math.abs(a_old - g_old) <= (a_old * LDBL_EPSILON) ) break;
   }
   F = (double) (phi / g);
   Fk = (double) (PI_2 / g);
   E =  (double) (0.5 * integral + 0.25 * sum * phi / g);
   Ek = (double) ((PI_4 / a) * sum / 2.0);
}
